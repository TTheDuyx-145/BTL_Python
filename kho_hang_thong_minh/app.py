...
def stock():
    ...
    selected_kho_req = request.args.get("kho", "ALL")
    q = (request.args.get("q") or "").strip()
    sort = (request.args.get("sort") or "qty_desc").strip()   # <-- mặc định: cao -> thấp
    ...
    # -------- Truy vấn & sắp xếp ----------
    if selected_kho == "ALL":
        # Gộp theo (kho, sp) + tổng số lượng
        base = (
            db.session.query(
                TonKho.id_kho.label("kho"),
                SanPham,
                func.coalesce(func.sum(TonKho.so_luong), 0).label("sl")
            )
            .join(SanPham, TonKho.id_san_pham == SanPham.id_san_pham)
            .group_by(TonKho.id_kho, SanPham.id_san_pham)
        )
        if q:
            base = base.filter(
                (SanPham.id_san_pham.ilike(like)) |
                (SanPham.ten_san_pham.ilike(like))
            )
        # Sắp xếp theo sl
        if sort == "qty_asc":
            base = base.order_by(func.sum(TonKho.so_luong).asc(), TonKho.id_kho, SanPham.id_san_pham)
        else:
            base = base.order_by(func.sum(TonKho.so_luong).desc(), TonKho.id_kho, SanPham.id_san_pham)

        rows = base.all()
        records = [(sp, int(sl or 0), kho) for kho, sp, sl in rows]

    else:
        # Danh sách sản phẩm (lọc theo q)
        sp_q = db.session.query(SanPham)
        if q:
            sp_q = sp_q.filter(
                (SanPham.id_san_pham.ilike(like)) |
                (SanPham.ten_san_pham.ilike(like))
            )
        sps = sp_q.order_by(SanPham.id_san_pham).all()

        # Map tồn theo kho đã chọn
        ton_rows = (
            db.session.query(TonKho.id_san_pham, TonKho.so_luong)
            .filter(TonKho.id_kho == selected_kho)
            .all()
        )
        ton_map = {sid: int(sl or 0) for sid, sl in ton_rows}

        records = [(sp, ton_map.get(sp.id_san_pham, 0), selected_kho) for sp in sps]
        # Sắp xếp theo sl
        records.sort(key=lambda r: r[1], reverse=(sort != "qty_asc"))
    ...
    return render_template(
        ...
        selected_kho=selected_kho,
        q=q,
        sort=sort,                     # <-- trả sort cho template
    )
